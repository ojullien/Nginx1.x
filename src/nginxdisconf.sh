...
## -----------------------------------------------------
## Disable a NGINX configuration file.
## -----------------------------------------------------
...
Conf::disable() {
    ...
    if (($# != 3)) || [[ -z "$1" ]] || [[ -z "$2" ]] || [[ -z "$3" ]]; then
        Console::error "Usage: Conf::disable <available conf directory> <enabled conf directory> <conf file>"
        exit 1
    fi
    ...
    # Conf already disabled
    if [[ ! -f "${sEnableDir}/${sFile}.conf" ]]; then
        Console::notice -n "Conf ${sFile} is "
...
if (($# < 1)) || [[ -z "$1" ]]; then
    ...
    echo -e "\tDisable a NGINX configuration file."
...
do
    Conf::disable "${m_DIR_AVAILABLE}" "${m_DIR_ENABLE}" "${1}"
    iReturn=$?
    if ((0!=iReturn)); then
        exit ${iReturn}
    fi
